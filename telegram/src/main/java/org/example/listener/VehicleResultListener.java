...
public class VehicleResultListener {
    private final ObjectMapper objectMapper;
    private final ResultMessageService resultMessageService;

    @KafkaListener(topics = "vehicle_req")
    public void listen(String message) {
        try {
            ResultMessage resultMessage = objectMapper.readValue(message, ResultMessage.class);
            resultMessageService.sendVehicleResult(resultMessage);
        } catch (Exception exception) {
            throw new RuntimeException("Не удалось обработать сообщение машины", exception);
        }
    }
}
